...
class HotkeyGUI:
    def __init__(self, root):
        ...
        # Global controllers for macOS input handling
    ...
    def _click_at(self, coords):
        """Move cursor to coords and perform a left click (macOS)."""
        try:
    ...
    def _right_click_at(self, coords):
        """Move cursor to coords and perform a right click (macOS)."""
        try:
            x, y = (int(coords[0]), int(coords[1]))
            self.mouse_controller.position = (x, y)
            threading.Event().wait(0.05)
            self.mouse_controller.press(pynput_mouse.Button.right)
            threading.Event().wait(0.05)
            self.mouse_controller.release(pynput_mouse.Button.right)
        except Exception as e:
...
"""
        ...
        # Type amount
        try:
            self.keyboard_controller.type(amount)
        except Exception:
            pass
    ...
    def register_hotkeys(self):
        """Register global hotkeys using pynput (macOS)."""
        try:
            # Stop existing listener if any
            if hasattr(self, 'global_key_listener') and self.global_key_listener:
                try:
                    self.global_key_listener.stop()
                except Exception:
                    pass
            # Start a new global listener
            self.global_key_listener = pynput_keyboard.Listener(on_press=self._on_global_key)
    ...
    def _on_global_key(self, key):
        """Handle global hotkeys mapped in self.hotkeys."""
        try:
            ...
            if key_str == self.hotkeys.get('toggle_loop', ''):
                self.toggle_main_loop()
            elif key_str == self.hotkeys.get('toggle_layout', ''):
                self.toggle_layout()
            elif key_str == self.hotkeys.get('exit', ''):
                self.exit_app()
            elif key_str == self.hotkeys.get('toggle_minimize', ''):
                self.toggle_minimize_hotkey()
        except Exception:
            pass
    ...
    def pause_fishing(self):
        ...
        # Release mouse if clicking (macOS)
        if self.is_clicking:
            try:
                self.mouse_controller.release(pynput_mouse.Button.left)
            except Exception:
